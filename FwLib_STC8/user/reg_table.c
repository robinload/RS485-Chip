...
#define U16_DEF(x) ((int32_t)(uint16_t)(x))


/* ================= RAM Variables ================= */
/* These must be defined here so the Linker can find them */
/* ================= RAM Variables (Moved to XDATA) ================= */
...
__xdata int32_t reg_fir_filter_on; 
...
__xdata int32_t reg_can_rate;        
__xdata int32_t reg_can_send_interval;
__xdata int32_t reg_can_send_type;    
__xdata int32_t reg_stable_mark;      
__xdata int32_t reg_cal_trigger;      
...
const reg_desc_t reg_table[] = {
 ...
 /* ================= Core Parameters ================= */
 /* MB_ADDR | EE_ADDR | TYPE | RAM | DEFAULT | MIN | MAX | FLAGS */
...
void reg_load(const reg_desc_t *r) {
    ...
    /* --- FIXED DEBUG CALLS --- */
    // UART_SendString("Addr "); 
    // UART_SendInt(r->eeprom_addr);
    // UART_SendString(" Raw: "); 
    // UART_SendLong(v); 
    // UART_SendString("\r\n");
